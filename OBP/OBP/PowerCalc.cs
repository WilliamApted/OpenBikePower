...
namespace OBP
{
    ...
    {
        /// <summary>
        /// Calculates the cycling power with the given inputs.
        /// </summary>
        ...
        /// <param name="windSpeed">Current wind-speed, positive when a headwind and negative when tailwind.</param>
        ...
        public double GetPower(float velocity, float mass, float slope, float windSpeed, float airDensity, float rollResistanceCoefficient, iDragType dragType , float powerLoss = 0) 
        {
            return CompleteCalc(Gravity.GravitationalForce(slope, mass), RollResistance.Resistance(slope, mass, rollResistanceCoefficient), Drag.Calculate(airDensity, velocity, windSpeed, dragType), velocity, powerLoss);
        }
        ...
        /// <param name="dragForce">Force due to aerodynamic drag.</param>
        /// <param name="velocity">Current velocity.</param>
        /// <param name="powerLoss">Power loss (as a percentage, 5% -> 0.05)</param>
        /// <returns>Power in watts.</returns>
        private double CompleteCalc(double gravityForce, double rollingForce, double dragForce, double velocity, double powerLoss)
        {
